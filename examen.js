...
function concatenarStrings (a, b) {
  ...
  return  `${a} ${b}`;
  console.log(suamaCadenas);
}

console.log(concatenarStrings('Me llamo Javier;','¿necesitas ayuda?'));
...
let coche = {
  ...
  function contarPropiedades (coche, nombreObjeto) {
    for(let i = 0; i in coche){
      if (coche.hasOwn;
      console.log(empleado.nombre + (arma === ' kungfu' ? ' sabe' : ' tiene ') + arma);
  }
  }

  console.log(contarPropiedades(coche, "coche") );
